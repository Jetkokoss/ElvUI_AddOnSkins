...
S:AddCallbackForAddon("DBM-Core", "DBM-Core", function()
	...
	local backportVersion2 = DBM.ReleaseRevision >= 20220631000000 -- 9.2.14
	...
	local function skinBarsST(self)
		local db = E.db.addOnSkins

		for bar in self:GetBarIterator() do
			if not bar.injected then
				hooksecurefunc(bar, "Update", function()
					local sparkEnabled = DBTST.Options.Spark
					if not (db.DBMSkinHalf and sparkEnabled) then return end
					local spark = _G[bar.frame:GetName().."BarSpark"]
					spark:SetSize(12, ((bar.enlarged and DBTST.Options.HugeHeight or DBTST.Options.Height) * 3) - 2)
					local a, b, c, d = spark:GetPoint()
					spark:SetPoint(a, b, c, d, 0)
				end)
				hooksecurefunc(bar, "ApplyStyle", function()
					...
					if DBTST.Options.IconLeft then icon1.overlay:Show() else icon1.overlay:Hide() end
					if DBTST.Options.IconRight then icon2.overlay:Show() else icon2.overlay:Hide() end
